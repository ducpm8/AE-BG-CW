...
public class DataGrabP2_DBC {
	...
	public Map<String, List> doPlaceOrder(String fileName, P2SubmitData sbData)
    {
		...
		AmazonInventoryReport inventoryReport = new AmazonInventoryReport();
...
//                fileOut.close();
            	...
            	List<WebElement> popup = driver.findElements(By.xpath("//a[contains(@data-role,'layer-close')]"));
            	if (popup.size() > 0) {
            		popup.get(0).click();
            		Thread.sleep(5000);
            	}
...
//			                			if (driver.findElements(By.id("qp-shippingInfo-edit")).size() > 0) {
//			                    	    	driver.findElement(By.id("qp-shippingInfo-edit")).click();
//			                    	    }
				                		...
				                		if (cellVal.matches("\\A\\p{ASCII}*\\z")) {
				                			...
				                			for (int idx=0;idx<20;idx++) {
				                				if (driver.findElement(By.name("contactPerson")).getAttribute("value").equals(cellVal)) {
				                					break;
				                				} else {
				                					driver.findElement(By.name("contactPerson")).clear();
				                					driver.findElement(By.name("contactPerson")).sendKeys(cellVal);
...
//				                			}
			                			...
			                			if (!ConstVal.stateMap.containsKey(cellHS.getStringCellValue())) {
			                				...
			                				tmpState = cellHS.getStringCellValue();
			                				
			                				if (tmpState.contains(" ")) {
			                					
			                					if (tmpState.split(" ").length == 2) {
			                					
				                					tmpState = tmpState.toLowerCase();
				                					
				                					firstChar = tmpState.split(" ")[0];
				                					
				                					secondChar = tmpState.split(" ")[1];
				                					
				                					tmpState = firstChar.substring(0, 1).toUpperCase() + firstChar.substring(1) + " " + secondChar.substring(0, 1).toUpperCase() + secondChar.substring(1);
			                					}
			                				} else {
			                					tmpState = tmpState.substring(0, 1).toUpperCase() + tmpState.substring(1).toLowerCase();
			                				}
			                				
			                				stateFullName = tmpState;
				                		...
				                		driver.findElement(By.name("phoneCountry")).sendKeys("1");
...
//				                				throw new IllegalArgumentException("buyer-phone-number format incorrect");
//				                			} else {
//				                				driver.findElement(By.name("phoneCountry")).sendKeys(tel.split("-")[0]);
//				                				driver.findElement(By.name("phoneArea")).sendKeys(tel.split("-")[1]);
//				                				driver.findElement(By.name("phoneNumber")).sendKeys(tel.split("-")[2]);
//				                			}
//				                		} else {
//				                			driver.findElement(By.name("mobileNo")).sendKeys(tel);
//				                		}
				                        ...
				                        //calculate profit for current product
				                        
				                		String originalPrice = driver.findElement(By.xpath("//div[@class='orderPrice-detail multiple-lan util-clearfix']/p[1]")).getText();
				                		String shipFee = driver.findElement(By.xpath("//div[@class='orderPrice-detail multiple-lan util-clearfix']/p[2]")).getText();
				                		
				                        inventoryReport.setCostFromAE(originalPrice.replaceAll("[^\\d.]", ""));
				                        inventoryReport.setShippingCostEpacket(shipFee.replaceAll("[^\\d.]", ""));
				                        
				                        String imageName = String.valueOf(System.currentTimeMillis() % 1000).concat(".jpg");
				                		
				                		//Captcha blank
				                		if (driver.findElements(By.id("captcha-input")).size() > 0) {
				                			
				                			//Refresh captcha
				                			//driver.findElement(By.id("flush-catpcha-image")).click();
				                			//Thread.sleep(1000);
				                			
				                			//DBC
				                    	    //Start loop
				    					    boolean completed = true;
				    					    String captchaImgSource = driver.findElement(By.id("captcha-image")).getAttribute("src");
				    					    String captchaImgFile = "";
				    					    WebElement captchaImgc = driver.findElement(By.id("captcha-image"));
				    					    
				    					    while (completed) {
				    					    	try {
				    						    	captchaImgFile = shootWebElement(captchaImgc, driver, imagePath);
				    						    	
				    						    	//String captchaImgFile = downloadFile.downloadFile(captchaImgSource, imagePath, imageName);
				    							    ...
				    							    while (true) {
				    							    	...
				    							    	try {
				    							    		captVal = dbc.getPixel(captchaImgFile);
				    							    	} catch (Exception e) {
				    							    		System.out.println(e.getMessage());
				    							    	}
				    							    	
				    							    	if (!captVal.isEmpty()) {
				    							    		driver.findElement(By.id("captcha-input")).clear();
				    							    		driver.findElement(By.id("captcha-input")).sendKeys("");
				    							    		driver.findElement(By.id("captcha-input")).sendKeys(captVal);
				    							    		
				    							    		for (int idx=0;idx<20;idx++) {
				    							    			if (driver.findElement(By.id("captcha-input")).getAttribute("value").equals(captVal)) {
				    							    				break;
				    							    			} else {
				    							    				driver.findElement(By.id("captcha-input")).clear();
						    							    		driver.findElement(By.id("captcha-input")).sendKeys("");
				    							    		...
				    							    		js = "document.getElementById('place-order-btn').click();";
									                		...
									                		if (driver.findElements(By.id("captcha-input-message")).size() < 1) {
							                    	        	...
							                    	        	File fileCapt = new File(captchaImgFile);

							                    	    		if(fileCapt.delete()){
							                    	    			System.out.println(fileCapt.getName() + " is deleted!");
							                    	    		}else{
									                			...
									                			String classMess = driver.findElement(By.id("captcha-input-message")).getAttribute("class");
									                			String style = driver.findElement(By.id("captcha-input-message")).getAttribute("style");
								                    	        
								                    	        if (classMess.indexOf("hide") >= 0 && style.indexOf("block") < 0) {
								                    	        	completed = false;
								                    	        	...
								                    	        	driver.findElement(By.id("flush-catpcha-image")).click();
								                    	        	Thread.sleep(2000);
								                    	        	FileUtils.forceDelete(new File(captchaImgFile));
								                    	        	//imageName = String.valueOf(System.currentTimeMillis() % 1000).concat(".jpg");
								                    	        	captchaImgFile = shootWebElement(captchaImgc, driver, imagePath);
								                    	        }
									                		}
				    							    		...
				    							    		FileUtils.forceDelete(new File(captchaImgFile));
//				    							    		imageName = String.valueOf(System.currentTimeMillis() % 1000).concat(".jpg");
//				    							    		captchaImgFile = downloadFile.downloadFile(captchaImgSource, imagePath, imageName);
				    							    		captchaImgFile = shootWebElement(captchaImgc, driver, imagePath);
				    							    	}
				    							    }
				    							    if (dbcCount > 5) {
				    							    	throw new IllegalArgumentException("DBC incorrect more than 5 times");
				    							    }
				                			...
				                			//if (!captchaImgFile.isEmpty()) FileUtils.forceDelete(new File(captchaImgFile));
...
//				        	            	}
				                		...
				                		if (driver.getCurrentUrl().indexOf("icashier.alipay.com") > 0) {
				                			...
				                			} else if (driver.findElements(By.name("cvv2")).size() > 0) {
					                			doTheLoop = true;
					                    	    h = 0;
					                    	    while (doTheLoop) { 
					                    	        h = h+1;
					                    	        Thread.sleep(2000);
					                    	        if (h>30){
					                    	        	throw new IllegalArgumentException("CVV2 require");
					                    	        }
					                    	        if (!driver.findElement(By.name("cvv2")).getAttribute("value").isEmpty()){
					                    	            doTheLoop = false;
					                    	        } 
					        	            	}
				                			}
			                    	    ...
			                    	    if (headerList.size() > 0) {
			                    	    	...
			                    	    	} else if (headerList.get(0).getText().indexOf("Payment failed") >= 0) {
			                    	    		throw new IllegalArgumentException("Order Failed. " + driver.findElement(By.xpath("//div[@id='pay-fail-body']")).getText());
			                    	    	}
			                    	    }
				                        ...
				                        profit = profit + (Double.valueOf(inventoryReport.getAbsoluteListPrice()) - 
				                        		Double.valueOf(inventoryReport.getCostFromAE()) - 
				                        		Double.valueOf(inventoryReport.getShippingCostEpacket()) -
				                        		Double.valueOf(inventoryReport.getAmazonFee()));
...
//                fileOut.close();
            ...
            result.put("PROFIT", new ArrayList<String>(Arrays.asList(profit.toString())));
...
public String shootWebElement(WebElement element, WebDriver driver, String imagePath) throws IOException, IllegalAccessException  {
		...
		while (true) {
			try {
				...
				element = driver.findElement(By.id("place-order-btn"));
		        ...
		        //BufferedImage dest = img.getSubimage(Integer.valueOf(element.getAttribute("offsetTop")), Integer.valueOf(element.getAttribute("offsetLeft")), 100,30);
		
		        BufferedImage dest = img.getSubimage(p.getX()+36, 165, 100,   
		                                 30);
//		        
//		        BufferedImage dest = img.getSubimage(p.getX(), 284, width,   
//                        height);
		        
		        System.out.println("X cua iframe " + p.getX());
		        System.out.println("Y cua iframe " + p.getY());
		
		        ImageIO.write(dest, "png", screen);
		        
		        long millis = System.currentTimeMillis() % 1000;
		
		        compressedImageFile = new File(imagePath + File.separator + millis + ".png");
		
		        FileUtils.copyFile(screen, compressedImageFile);
		        
		        //compress
		        //File input = new File("digital_image_processing.jpg");
//		        BufferedImage image = ImageIO.read(f);
//		
//		        compressedImageFile = new File("D:\\test\\compress.jpg");  
...
//		        ios.close();
				...
				countErr++;
				System.out.println("Failed in shootWebElement");
				System.out.println(e.getMessage());
